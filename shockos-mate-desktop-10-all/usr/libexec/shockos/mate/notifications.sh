...
then
    dnd_button="--button=\"Do Not Disturb ON\"\!notifications-disabled-symbolic"
    dnd="TRUE"
else
    lines=$(cat ~/.cache/shockos-tmp/notifications-log | wc -l)
    for line_num in $(seq 1 $lines)
    do
        line="$(sed -n "$line_num"p ~/.cache/shockos-tmp/notifications-log)"
        if [[ "$counter" == "1" ]]
        then
            app_name="$line"
        elif [[ "$counter" == "2" ]]
...
eval "yad --window-icon=preferences-system-notifications-symbolic --geometry=${window_width}x${window_height}+${posx}+${posy} --fixed --undecorated --skip-taskbar --close-on-unfocus --list \
            ...
            --column=\"Text\" \
            ${yad_list[@]} \
            --buttons-layout=edge $dnd_button --button=\"Clear All\"\!edit-clear-all-symbolic"
ex=$?
...
then
    rm ~/.cache/shockos-tmp/notifications-log
fi
