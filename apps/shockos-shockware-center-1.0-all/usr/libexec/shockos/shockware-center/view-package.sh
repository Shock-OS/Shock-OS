...
mkdir -p ~/.cache/shockos-tmp/shockware-center

if [[ -f ~/.cache/shockos-tmp/shockware-center/indicator_instances ]]
then

    local instances=$(($(cat ~/.cache/shockos-tmp/shockware-center/indicator_instances)+1))

    echo "$instances" | tee ~/.cache/shockos-tmp/shockware-center/indicator_instances
...
if (($(cat ~/.cache/shockos-tmp/shockware-center/indicator_instances)>1)) #if other apps are being installed, keep the notification
then
    ...
    rm ~/.cache/shockos-tmp/shockware-center/indicator_instances

    kill "$(cat ~/.cache/shockos-tmp/shockware-center/indicator_killpid)"

    rm ~/.cache/shockos-tmp/shockware-center/indicator_killpid
...
echo "$package" | tee -a ~/.cache/shockos-tmp/shockware-center/apt-queue

until (($(lslocks | grep -c apt)==0)) && [[ "$(sed -n 1p ~/.cache/shockos-tmp/shockware-center/apt-queue)" == "$package" ]] #waits until /var/lib/dpkg/lock-frontend is not being held
do
...
if (($(cat ~/.cache/shockos-tmp/shockware-center/apt-queue | wc -l)<2)) #removes the queue file if no other operations are pending.
then

    rm ~/.cache/shockos-tmp/shockware-center/apt-queue

else

    sed -i 1d ~/.cache/shockos-tmp/shockware-center/apt-queue
